...
// can both call it without coordinating who owns the fixture setup.
async fn ensure_tenant_and_issuer(pool: &PgPool, tenant_id: &str, issuer_id: &str) {
    sqlx::query(
        "INSERT INTO tenants (id, partner_id) VALUES ($1, gen_random_uuid()) \
    ...
    .unwrap();
    sqlx::query(
        "INSERT INTO issuers (id, tenant_id, did) VALUES ($1, $2, $3) \
         ON CONFLICT (id) DO NOTHING",
    )
    .bind(issuer_id)
    .bind(tenant_id)
    .bind(format!("did:tdw:test.example.com:{issuer_id}"))
    .execute(pool)
    .await
    .unwrap();
}
...
async fn insert_status_list(pool: &PgPool, list_id: &StatusListId, issuer_id: &str) {
    ensure_tenant_and_issuer(pool, TEST_TENANT, issuer_id).await;
    sqlx::query(
...
async fn insert_credential_offer(
    ...
    ensure_tenant_and_issuer(pool, tenant_id, issuer_id).await;
    sqlx::query(
...
async fn well_formed_row_inserts_cleanly(pool: PgPool) {
    ...
    insert_credential_offer(&pool, &offer_id, TEST_TENANT, TEST_ISSUER).await;
...
async fn duplicate_status_list_index_is_rejected(pool: PgPool) {
    ...
    insert_credential_offer(&pool, &first_offer, TEST_TENANT, TEST_ISSUER).await;
    insert_issued_credential(
        &pool,
        &IssuedCredentialId::generate(),
        TEST_TENANT,
    ...
    insert_credential_offer(&pool, &second_offer, TEST_TENANT, TEST_ISSUER).await;
...
async fn same_index_in_different_lists_is_allowed(pool: PgPool) {
    ...
    insert_status_list(&pool, &first_list, TEST_ISSUER).await;
    let second_list = StatusListId::generate();
    insert_status_list(&pool, &second_list, TEST_ISSUER).await;
...
fn seeded_tenant_id() -> TenantId {
    TenantId::from_bare(TEST_TENANT).unwrap()
}

fn seeded_issuer_id() -> IssuerId {
    IssuerId::from_bare(TEST_ISSUER).unwrap()
}
