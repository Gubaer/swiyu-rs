...
|--------------------------|------------------------------------------|---------------------|
| Structured business data | DBMS (`swiyu_issuer_mgmt` schema)        | DBMS (same)         |
| Key material             | DBMS (`swiyu_issuer_keystore` schema), AEAD-encrypted per tenant | HSM                 |
...
Tenants, admin users, API tokens (hashed), issuers, issuer DIDs and their Trust Statement references, credential type configurations, and branding. Low write rate, low volume, frequently read. Reconsider only if the catalogue grows enough to make these tables hot — not a realistic horizon for the cantonal target scenario.
...
One row per status list per issuer, with the bitstring as `bytea` and a version/last-updated stamp. The signed status-list credential (what the wallet fetches at the well-known URL) is regenerated on each change and stored alongside, or cached in front of the DB.

Bit flips happen inside the same transaction as the credential issuance or revocation; concurrency is the DBMS's problem.

Footprint is small: a standard 131 072-bit BitstringStatusList is 16 KB pre-compression.

Reconsider on read latency at the public status-list endpoint under wallet fan-out (every wallet checking status during verification can hammer it). The first response is usually a CDN in front of the existing endpoint, not a different backend.

### Hot / ephemeral state

OIDC pre-authorised codes, transaction codes, nonces, in-flight wallet sessions. Whether this is a single `oidc_ephemeral` table with a kind discriminator or several focused tables is a design-level decision later.

Every row carries an `expires_at`; expired rows are removed by a periodic cleanup (pg_cron, an external sweeper, or a Postgres background worker).
...
Append-only. No `UPDATE` / `DELETE` in normal operation; cleanup only via a retention policy. Schema captures: tenant, issuer (nullable for tenant-level events), actor kind and id, action, target kind and id, timestamp, and a JSONB details payload for the variable bits.
...
### Pre-production maturity (alpha, beta) and intermediate

Private keys are stored AEAD-encrypted in the `swiyu_issuer_keystore` schema, with a per-tenant KEK held by the deployment's orchestrator (Kubernetes Secret, systemd-creds, etc.). Alpha/beta uses an auto-provisioned dev KEK; intermediate maturity uses the same code path with a real orchestrator-managed KEK. The filesystem-based keystore from `swiyu-didtool` is **not** used at runtime by `swiyu-issuer`.

### Production maturity (prod-1 onwards)

Keys live in an HSM (or HSM-backed KMS) operated by the canton. The keystore row stores an opaque HSM key handle; no private material enters the application or the database. Signing is performed by sending the to-be-signed bytes to the HSM and receiving the signature back.

### Pre-production → production transition

HSM keys are typically non-exportable by policy. The transition is therefore not a migration of keys *into* the HSM; it is a **key rotation**:
...
A fresh issuer that never had a pre-production presence skips the rotation entirely and starts on HSM-generated keys.

A single deployment may legitimately hold a mix of DB-backed and HSM-backed issuers during the transition window.
...
- Keys live in `swiyu_issuer_keystore`, AEAD-encrypted with a dev KEK. Throwaway.
...
- HSM keys generated for every active issuer; the previous DB-stored encrypted keys retired via DID rotation.
...
- **Signing path** abstracts over the key backend. A single trait (or equivalent abstraction) with two implementations: DB-backed decrypt-and-sign (alpha through intermediate) and HSM (production). Selection is per deployment configuration, possibly per issuer during the DB-backed → HSM transition window.
- **Key references** stored on issuer records are abstract enough to refer to either a DB-backed encrypted-key row or an HSM key handle; the signer interprets the reference.
...
- **Hot/ephemeral schema.** One table with a kind discriminator vs. several focused tables.
- **HSM provider on the canton side.** Affects how key references are encoded on issuer records and which signing protocol the signer abstraction speaks (PKCS#11, KMIP, vendor SDK, …).
...
- **What to store of an issued credential.** Full signed compact JWT vs. claims + status-list pointer + issuance metadata only.
