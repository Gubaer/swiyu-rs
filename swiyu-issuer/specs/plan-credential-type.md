...
- ~~**End-to-end smoke examples target a vct the dev tenant no longer seeds.**~~ **Done.** `credential_lifecycle_smoke.rs` and `credential_status_lifecycle_smoke.rs` now discover the dev-seeded dummy credential type via `GET /api/v1/credential-types` (filtering by `vct = "urn:dummy:dummy-credential"`), assign it to the fresh issuer they create, send `credential_type_id` on the offer-create body, and shape claims to match the dummy schema (`{ first_name, last_name }`). The wallet-facing OIDC `/credential` call still passes `vct`, but the value comes from the discovered type rather than a hardcoded constant.
