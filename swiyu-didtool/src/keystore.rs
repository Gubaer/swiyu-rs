...
pub struct KeyStoreEntry {
    /// 12-character BLAKE3 hex prefix of the DID string; names the entry's directory on disk.
    hash: String,
    /// The full DID this entry holds keys for.
    did: String,
    /// Absolute path to the entry's directory, holding `did.txt` and the numbered key
    /// snapshots (`0001/`, `0002/`, …).
    entry_dir: PathBuf,
}
...
impl KeyStoreEntry {
    ...
    /// Writes a new snapshot version with `staged` and returns its version number.
    ///
...
impl KeyStore {
    ...
    ///
    /// No filesystem I/O is performed; the keys are written to disk later by
    /// [`KeyStore::commit`], once the full DID is known.
    ...
    /// # Errors
    ///
    /// Returns [`KeyStoreError::AlreadyExists`] if an entry for this DID already exists.
    ...
    ///
    /// Reads `did.txt` and verifies it matches `did` before returning the entry.
    ///
    /// # Errors
    ///
    /// Returns [`KeyStoreError::Mismatch`] if the subdirectory exists but `did.txt` holds a
    /// different DID (hash collision or corruption).
